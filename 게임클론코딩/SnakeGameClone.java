...
public class SnakeGameClone {
	...
	static class MyFrame extends JFrame {
		...
		static int dir = 3; // 이동방향 -  0 : 위 // 1 : 아래 // 2 : 왼쪽 // 3 : 오른쪽
		...
		public MyFrame(String title) {
			super(title);
			this.setSize(400,500); // 크기
			this.setVisible(true); // 이 코드가 있어야 Ui가 보임
			this.setDefaultCloseOperation( // 이 코드가 있어야 Ui를 닫을수있음.
					JFrame.EXIT_ON_CLOSE);
			initUI();
            makeSnakeList(); // 뱀 몸통
            startTimer(); // start timer
            setKeyListener(); // 키보드 입력
            makeFruit(); // 열매
		}
		void removeFruit() {
			for(int i=0; i<20; i++) {
		...
		void makeSnakeList() {
            snake.add(new XY(10,10)); // 뱀 머리
            snake.add(new XY(9,10)); // 뱀 몸통
            snake.add(new XY(8,10)); // 뱀 꼬리
		}
...
// Center 영역 ================================================================
			...
			for(int i=0; i<20; i++) { // 행 - 세로
				for(int j=0; j<20; j++) { // 열 - 가로
					map[i][j] = 0; // 초기화
					panels[i][j] = new JPanel(); // 실제 패널이 만들어짐
